...
def parse_daily(forecast_list):
    ...
    for date, d in by_date.items():
        ...
        day_of_week = WEEKDAY_VI[dt_obj.weekday() if dt_obj.weekday() < 6 else 0] # Adjusting for WEEKDAY_VI starting with Sunday
        # Python weekday: 0=Monday, ..., 6=Sunday
        # WEEKDAY_VI: 0=Sunday, 1=Monday, ...
...
def process_weather_data(data):
    ...
    if aqi and 'list' in aqi and len(aqi['list']) > 0:
        ...
        processed_aqi = {
            ...
            'desc': info['desc'],
            'color': info['color'],
            'pm25': aqi_item['components']['pm2_5'],
            'co': aqi_item['components']['co'],
    ...
    stats = {}
    if daily:
        stats['avg_temp'] = round(sum((d['temp_max'] + d['temp_min']) / 2 for d in daily) / len(daily), 1)
        stats['total_rain'] = round(sum(item.get('rain', {}).get('3h', 0) for item in forecast.get('list', [])), 1)
    ...
    return {
        ...
        'lucide_icon': icon_to_lucide(current['weather'][0]['icon']),
...
def get_location_by_ip():
    try:
        # Sử dụng API miễn phí để lấy vị trí từ IP
    ...
    return "Ho Chi Minh City" # Mặc định nếu lỗi
