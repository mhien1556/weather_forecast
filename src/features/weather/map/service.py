...
from src.services.weather_api import fetch_current
from src.common.units import get_units, convert_temp, convert_pressure, convert_wind_from_ms
...
_OWM_TILE_MAX_PAST = 5 * 24 * 3600
_OWM_TILE_MAX_FUTURE = 5 * 24 * 3600

_COMPASS = ['N', 'NNE', 'NE', 'ENE', 'E', 'ESE', 'SE', 'SSE', 'S', 'SSW', 'SW', 'WSW', 'W', 'WNW', 'NW', 'NNW']


def is_timestamp_in_owm_range(unix_ts: int) -> bool:
    now = int(datetime.now().timestamp())
    return (now - _OWM_TILE_MAX_PAST) <= unix_ts <= (now + _OWM_TILE_MAX_FUTURE)
...
def build_weather_tile_url(layer_key: str, api_key: str, unix_ts: int | None = None) -> str:
    ...
    if unix_ts is not None and is_timestamp_in_owm_range(unix_ts):
...
_probe_cache: dict = {}
_PROBE_CACHE_TTL = timedelta(minutes=30)


def _probe_tile_valid(layer_key: str, api_key: str, unix_ts: int) -> bool:
    url = (
        f'https://tile.openweathermap.org/map/{layer_key}/1/1/1.png'
        f'?appid={api_key}&date={unix_ts}'
    )
    try:
...
def build_timeline(hourly: list, api_key: str = '') -> list[int]:
    ...
    if hourly:
        ...
        times = [t for t in times if past_start <= t <= max_ts]
        if times:
            return times
...
def main_metric_value(snapshot: dict, layer_key: str) -> tuple[str, str]:
    ...
    # Kiểm tra loại dữ liệu để chuyển đổi đơn vị
    if field == 'temp':
        return str(convert_temp(val, u['unit_temp'])), ('°F' if u['unit_temp'] == 'F' else '°C')
    if field == 'pressure':
        return str(convert_pressure(val, u['unit_pressure'])), u['unit_pressure']
    if field == 'wind_speed':
        return str(convert_wind_from_ms(val, u['unit_wind'])), u['unit_wind']
    if field == 'clouds':
        return str(val), '%'
    if field == 'rain':
        return str(val), 'mm'
...
def get_hourly_weather(api_key: str, city: str = 'Hanoi') -> dict:
    cache_key = f'map:{city.lower().strip()}'
