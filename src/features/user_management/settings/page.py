...
def register():
    ...
    def settings_page():
        ...
        # Định cấu hình màu nền nền đen đặc/trắng đặc cho toàn bộ trang gốc
        from src.common.units import get_units
        is_dark_mode = get_units().get('theme', 'dark') == 'dark'
        page_bg = 'background-color: #121212;' if is_dark_mode else 'background-color: #f4f6f9;'

        with ui.element(_TAG).classes('app-container').style(page_bg):
            hero_background(weather if not weather.get('error') else None)
            navbar('/settings')

            with ui.element(_TAG).classes('page-content content-wrapper'):
                with ui.element(_TAG).classes('page-header').style('margin-bottom: 1rem; color: inherit;'):
